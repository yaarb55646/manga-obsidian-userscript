...
```js
STATUS_MAP = {
    '连载': '连载中', '连载中': '连载中', '連載中': '连载中',
    '完结': '已完结', '完結': '已完结', '已完结': '已完结', '已完結': '已完结',
}
// 用于归一化各站点的连载状态文字
...
```js
{
    ...
    是否完结: string,   // "连载中" / "已完结" / ""（空则导出时默认"连载"）
...
- 话数/完结状态：`.last-update span.v-middle`
  - 连载格式：`更新至 XX 话` → 总话数=XX，是否完结=连载中
  - 完结格式：`[完结] 共 XX 话` → 总话数=XX，是否完结=已完结
